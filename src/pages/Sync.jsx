...
import { addAtom, checkedbox, dataAtom, deleteAtom, editAtom } from '../atoms/todo'
        ...
        <section className='flex flex-wrap items-center justify-center gap-[20px]'>
           {
            filteredData.map((e) => {
              return(
                <div key={e.id} className='flex flex-col items-center justify-center gap-[10px] border-[5px] border-[white] p-4 shadow-2xl rounded-2xl w-70 h-70'>
                  <h1>{e.name}</h1>
                  <p className={e.status ? "text-green-600" : "text-red-600"}>{e.status ? "Active" : "Inactive"}</p>
                  <div className='flex items-center justify-center gap-[10px]'>
                    <DeleteOutlined  onClick={() => deleteUser(e.id)} />
